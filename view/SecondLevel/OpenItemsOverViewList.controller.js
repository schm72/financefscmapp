...
com.springer.financefscmapp.util.Controller.extend("com.springer.financefscmapp.view.SecondLevel.OpenItemsOverViewList", {
	...
	onInit: function() {
		...
		this._oTable = this.byId("idOIListTable");
		this._UserPreferences = sap.ui.getCore().getModel("UserPreferences");
		this._vUserIdWLmode = this.getView().byId("UserIdWLmode");
		this._vListViewChanger = this.byId("oiListViewChanger");
		this._vListDrpd = this.byId("ioListDrpd");
	...
	onRouteMatched: function(evt) {
		...
		if (evt.getParameter("name") !== "_OpenItemsOverViewList") {
			return;
		}
	...
	onAfterShow: function() {
		...
		if (this._UserPreferences._vVisible_oi === false || document.body.clientWidth > 9001) {
		...
		if (this._UserPreferences.enteredOIListView == undefined) {
			this._UserPreferences.enteredOIListView = true;
	...
	checkNoDataDetails: function() {
		...
		var that = this;
		var items = this.byId("idOIListTable").getItems();
		if (items.length < 1) {
			var that = this;
	...
	handleViewChangeTop: function() {
		var vId = null;
		this._UserPreferences._vVisible_oi = true;
		...
		for (var i = 0; i < Columns.length; i++) {
			...
			if (vId.indexOf("idHiddenColmn") > -1) {
				if (Columns[i].getVisible() === true) {
					...
					this._UserPreferences._vVisible_oi = false;
					this._vListViewChanger.setIcon("sap-icon://drill-down");
					if (this.showTexts === true) {
						this._vListViewChanger.setText("Extend View");
					}
